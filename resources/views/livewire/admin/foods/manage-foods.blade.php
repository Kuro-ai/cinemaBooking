        ...
        <form wire:submit.prevent="{{ $isEditing ? 'update' : 'store' }}">
            ...
            <div class="mt-4 mb-4">
                @if ($image)
                    ...
                    <img src="{{ $image->temporaryUrl() }}" class="w-48 h-48 rounded-md border border-gray-300 dark:border-gray-600">
                @elseif ($isEditing && $existingImagePath)
                    <p class="text-gray-600 dark:text-gray-300 mb-2">Existing Image:</p>
                    <img src="{{ asset('storage/' . $existingImagePath) }}" class="w-48 h-48 rounded-md border border-gray-300 dark:border-gray-600">
                @else
                ...
                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                    <td class="border border-gray-400 dark:border-gray-600 px-4 py-2 h-20">
                        <div class="flex justify-center items-center">
                            @if ($food->image_path)
                                <img src="{{ asset('storage/' . $food->image_path) }}" class="w-16 h-16 rounded-md">
                            @else
                                <span>No Image</span>
                            @endif
                        </div>
